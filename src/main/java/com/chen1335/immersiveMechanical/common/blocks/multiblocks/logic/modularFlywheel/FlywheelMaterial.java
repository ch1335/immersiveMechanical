...
public record FlywheelMaterial(HolderSet<Block> holderSet, int maxEnergyStored) {
    ...
    public static void bootstrap(BootstrapContext<FlywheelMaterial> context) {
        register(context, "copper", Tags.Blocks.STORAGE_BLOCKS_COPPER, 18_200_000);
        register(context, "iron", Tags.Blocks.STORAGE_BLOCKS_IRON, 16_000_000);
        register(context, "gold", Tags.Blocks.STORAGE_BLOCKS_GOLD, 39_300_000);
        register(context, "steel", IETags.getTagsFor(EnumMetals.STEEL).storage, 16_000_000);
        register(context, "aluminum", IETags.getTagsFor(EnumMetals.ALUMINUM).storage, 5_500_000);
        register(context, "nickel", IETags.getTagsFor(EnumMetals.NICKEL).storage, 18_100_000);
        register(context, "uranium", IETags.getTagsFor(EnumMetals.URANIUM).storage, 38_700_000);
        register(context, "silver", IETags.getTagsFor(EnumMetals.SILVER).storage, 21_300_000);
        register(context, "lead", IETags.getTagsFor(EnumMetals.LEAD).storage, 23_100_000);
        register(context, "constantan", IETags.getTagsFor(EnumMetals.CONSTANTAN).storage, 18_100_000);
        register(context, "electrum", IETags.getTagsFor(EnumMetals.ELECTRUM).storage, 29_500_000);
    }
